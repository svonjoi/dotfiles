#!/usr/bin/env python3
# https://github.com/ant-arctica/rofi-bookmarks
# отредачил чтобы можно было открывать через кути биндом
...
if __name__ == "__main__":
    ...
    if environ.get('ROFI_RETV') == '10':
        subprocess.Popen(["firefox", environ['ROFI_INFO']] + prof, close_fds=True, start_new_session=True, stdout=subprocess.DEVNULL)
    elif environ.get('ROFI_RETV') == '11':
        subprocess.Popen(["qutebrowser", environ['ROFI_INFO']] + prof, close_fds=True, start_new_session=True, stdout=subprocess.DEVNULL)
    else:
        ...
        # https://github.com/davatorium/rofi/blob/next/doc/rofi-script.5.markdown#passing-mode-options
        print("\x00use-hot-keys\x1ftrue")  # enable custom bindings

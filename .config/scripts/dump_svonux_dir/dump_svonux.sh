...
function main() {
	...
	if [ $dump_root_files -eq 1 ] || [ $make_restore -eq 1 ]; then
		...
		# the script is done with paths pointing dirs of files; Each chmod/chown statment is executed
		# NON recursively, so if this array is not done well it shouldn be a problem
		excluded_real_paths=(
			'/etc'
			'/etc/systemd'
			'/etc/udev'
		...
		readarray -t dump_filepaths < <(find "$targetdir_root") || {
